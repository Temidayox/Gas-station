...
function generateSecureCylinderCode() {
  ...
  // Pattern: Letter + Number + Letter + Number + Letter
  // This creates more unpredictable sequences
  const positions = [
    { type: 'letter', value: randomValues[0] % 26 },
    { type: 'number', value: randomValues[1] % 10 },
    { type: 'letter', value: randomValues[2] % 26 },
    { type: 'number', value: randomValues[3] % 10 },
    { type: 'letter', value: randomValues[4] % 26 }
  ]
  
  positions.forEach(pos => {
    if (pos.type === 'letter') {
      code += letters[pos.value]
    } else {
      code += numbers[pos.value]
    }
  })
